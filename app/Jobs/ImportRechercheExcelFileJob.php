...
class ImportRechercheExcelFileJob implements ShouldQueue
{
        ...
        try
        {
            ...
            DB::transaction(function () use (&$totalToUpload, &$totalUpload, &$data, &$report)
            {
                ...
                for ($i=1; $i < count($data); $i++)
                {
                    ...
                    $identifiant    = utf8_encode(trim($row[0]));
                    $designation    = utf8_encode(trim($row[1]));
                   
                    if(!empty($identifiant) && !empty($designation))
                    {
                        $item = Recherche::where('identifiant', $identifiant)->first();
