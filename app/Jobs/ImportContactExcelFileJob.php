...
class ImportContactExcelFileJob implements ShouldQueue
{
        ...
        try
        {
            ...
            DB::transaction(function () use (&$totalToUpload, &$totalUpload, &$data, &$report)
            {
                ...
                for ($i=1; $i < count($data); $i++)
                {
                    ...
                    $lien       = utf8_encode(trim($row[1]));
                   
                    if(!empty($id_lvdc) && !empty($lien))
                    {
                        $item = Contact::where('id_lvdc', $id_lvdc)->first();

                        if(isset($item))
                        {
                            $item->lien     = !empty($lien) ? $lien : null;
                            $item->save();
                        }
